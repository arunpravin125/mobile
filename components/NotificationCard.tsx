...
const NotificationCard = ({ notification, onDelete }: notificationCardProps) => {
    ...
    const getNotificationIcon = () => {
        switch (notification.type) {
            ...
            case 'follow':
                return <Feather name='user-plus' size={20} color={"#178f63"} />
                ...
                <View className='flex-1' >
                    ...
                    {notification.post && (
                        <View className='bg-gray-50 rounded-lg p-3 mb-2' >
                            <Text className='text-gray-700 text-sm mb-1' numberOfLines={3} >{notification.post.content}</Text>
                            {notification.post.image && (
                                <Image source={{ uri: notification.post.image }} className='w-full h-32 rounded-lg mt-12' resizeMode='cover' />
                            )}
                    ...
                    {notification.comment && (
                        <View className='bg-blue-50 rounded-lg p-3 mb-2'>
                            ...
                            <Text className='text-gray-700 text-sm' numberOfLines={2}>&ldquo;{notification.comment.content}&ldquo;</Text>
                        </View>
                    )}
